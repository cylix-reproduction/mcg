...
SOURCE_DIR = "./ILSVRC2012"
TARGET_DIR = "./data/meta_imagenet"
...
def setup_train_data():
    ...
    train_source = os.path.join(SOURCE_DIR, "Data", "CLS-LOC", "train")
    ...
    if not os.path.exists(train_source):
        ...
        if os.path.exists(extracted_content):
            shutil.move(extracted_content, train_source)
    ...
    class_folders = [f for f in os.listdir(train_source) if os.path.isdir(os.path.join(train_source, f))]
...
def setup_val_data():
    ...
    val_source = os.path.join(SOURCE_DIR, "Data", "CLS-LOC", "val")
    ...
    if not os.path.exists(val_source):
        ...
        # Move extracted content to correct location
        extracted_content = os.path.join(temp_val_dir, "Data", "CLS-LOC", "val")
        if os.path.exists(extracted_content):
            shutil.move(extracted_content, val_source)
            # Clean up temp directory
            shutil.rmtree(temp_val_dir, ignore_errors=True)
    
    # Extract devkit to get validation ground truth
    devkit_dir = os.path.join(SOURCE_DIR, "devkit")
    if not os.path.exists(devkit_dir):
        ...
        if os.path.exists(extracted_content):
            shutil.move(extracted_content, devkit_dir)
            # Clean up temp directory
            shutil.rmtree(temp_devkit_dir, ignore_errors=True)
    
    # Load validation ground truth
    val_meta_path = os.path.join(devkit_dir, "data", "ILSVRC2012_validation_ground_truth.txt")
    if not os.path.exists(val_meta_path):
        # Try alternative path
        val_meta_path = os.path.join(SOURCE_DIR, "ILSVRC2012_devkit_t12", "data", "ILSVRC2012_validation_ground_truth.txt")
        if not os.path.exists(val_meta_path):
    ...
    if not os.path.exists(meta_path):
        # Try alternative path
        meta_path = os.path.join(SOURCE_DIR, "ILSVRC2012_devkit_t12", "data", "meta.mat")
        if not os.path.exists(meta_path):
            print("Meta file not found")
            return False
        
    meta = sio.loadmat(meta_path, squeeze_me=True)['synsets']
    ilsvrc_classes = [meta[i][0] for i in range(1000)]  # First 1000 classes
